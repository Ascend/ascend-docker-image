...
if [[ $(arch) == "x86_64" ]]; then
    cp -rf /usr1/package330/tensorflow-1.15.0-cp37-cp37m-manylinux2010_$(arch).whl .
else
    cp -rf /usr1/package330/tensorflow-1.15.0-cp37-cp37m-manylinux2014_$(arch).whl .
fi
...
cp -rf /usr1/package330/ResNet50_ID0058_for_TensorFlow .
...
if [ $arch == "x86_64" ];then
    ...
    # DOCKER_BUILDKIT=1  docker build . -t tensorflow-modelzoo:910b-ubuntu18.04-x64
else
    rm -f Ascend-cann-*-$(arch).run
    cp -rf /usr1/package330/Ascend-cann-toolkit_6.3*-$(arch).run .
    cp -rf /usr1/package330/Ascend-cann-tfplugin_6.3*-$(arch).run .
    ...
    # cp -rf /usr1/package330/Ascend-cann-toolkit_6.1*-$(arch).run .
    # cp -rf /usr1/package330/Ascend-cann-tfplugin_6.1*-$(arch).run .
    # DOCKER_BUILDKIT=1  docker build . -f Dockerfile_aarch64 -t tensorflow-modelzoo:910b-ubuntu18.04-arm64
fi