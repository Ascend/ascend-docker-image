...
if [ $arch == "x86_64" ];then
    DOCKER_BUILDKIT=0 docker build -t ascend-pytorch:centos7-x64 --build-arg BASE_VERSION=centos7-x64 .  || exit 1
    DOCKER_BUILDKIT=0 docker build -t ascend-pytorch:ubuntu18.04-x64 --build-arg BASE_VERSION=ubuntu18.04-x64 . || exit 1
else
    DOCKER_BUILDKIT=0 docker build -t ascend-pytorch:centos7-arm64 --build-arg BASE_VERSION=centos7-arm64 . || exit 1
