...
if [ $arch == "x86_64" ];then
    ...
    DOCKER_BUILDKIT=0 docker build -t ascend-toolkit:centos7-x64 --build-arg BASE_VERSION=centos7-x64 . || exit 1
    ...
    cp -rf /usr1/package930/Ascend-cann-kernels-910_7.0.RC1_linux.run .
    cp -rf /usr1/package930/Ascend-cann-kernels-910b_7.0.RC1_linux.run .
    DOCKER_BUILDKIT=0 docker build -t ascend-toolkit:centos7-arm64 --build-arg BASE_VERSION=centos7-arm64 . -f Dockerfile_aarch64|| exit 1
