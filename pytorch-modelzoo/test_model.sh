...
function start_test_model() {
    cd ~/samples/Resnet50_Cifar_for_PyTorch
    ...
    while true;do
        ...
        if [ "$(grep -c "THPModule_npu_shutdown success" ~/samples/Resnet50_Cifar_for_PyTorch/test/output/0/train_0.log)" -gt 0 ] && [ "$(grep -c "Saving checkpoint at 2 epochs" ~/Resnet50_Cifar_for_PyTorch/test/output/0/train_0.log)" -gt 0 ];then
            sleep 20
