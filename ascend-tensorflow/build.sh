...
if [ $arch == "x86_64" ];then
    ...
    DOCKER_BUILDKIT=1 docker build -t ascend-tensorflow:${version}-centos7.6-arm64 -f Dockerfile_aarch64 --build-arg BASE_VERSION=${version}-centos7.6-arm64 . || exit 1
    DOCKER_BUILDKIT=1 docker build -t ascend-tensorflow:${version}-ubuntu18.04-arm64 -f Dockerfile_aarch64 --build-arg BASE_VERSION=${version}-ubuntu18.04-arm64 . || exit 1
fi